...
from config import DB_CONFIG  # ➤ содержит словарь подключения к PostgreSQL
...
def fetch_inberlin_listings(seen_ids):
    ...
    response.raise_for_status()
    html = response.json().get("searchresults", "")
    ...
    for flat in flats:
        ...
        if img_tag and "style" in img_tag.attrs:
            ...
            if match and "flat-dummy.jpg" not in match.group(1):
